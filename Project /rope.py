...
def point_constraint(particle1, x2, y2):
    # calculate the difference between current projected position and target
    correction_x1 = x2 - particle1.px
    correction_y1 = y2 - particle1.py
    # return corrections to lock particle at target position
...
def distance_constraint(particle1,
                        particle2,
                        constraint_distance):
    # calculate current distance between projected positions
    current_dist = distance(particle1.px, particle1.py, particle2.px, particle2.py)
    
    # calculate how far off we are from the constraint distance
    diff = current_dist - constraint_distance
    
    # avoid division by zero
    if current_dist > 0.0001:
        # calculate normalized direction from particle1 to particle2
        dir_x = (particle2.px - particle1.px) / current_dist
        dir_y = (particle2.py - particle1.py) / current_dist
        
        # split correction equally between both particles
        correction_x1 = dir_x * diff * 0.5
        correction_y1 = dir_y * diff * 0.5
        correction_x2 = -dir_x * diff * 0.5
        correction_y2 = -dir_y * diff * 0.5
    else:
        correction_x1 = 0.0
        correction_y1 = 0.0
        correction_x2 = 0.0
